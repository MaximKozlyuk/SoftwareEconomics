...
def calc_var_spends(arr):
    ...
    headers = ["Наименование расходов", "Сумма (руб)"]
    rows.append(headers)
    zp = "Основная зарплата специалистов %s процент от стоимости тиражируемого продукта" % (round_money(arr[0]))
...
print_money("Стоимость продажи одной копии системы:", one_copy_cost)
...
print("Объем выпуска, при котором достигается точка безубыточности (нулевой уровень прибыли), определяется по формуле:")
print("x0 = a / (s - b) =", math.ceil(v_zero_point))

print("\nВыводы")
print("В течение месяца фирме необходимо подготовить и продать минимум", math.ceil(v_zero_point), "копий программного"
...
print("2.2 Расчет договорной цены тиражируемой системы при заданном объеме рынка продаж:")
...
print("2.3 Определение дополнительного объема продаж при заданном уровне прибыли")
Xd = (additional_profit + fixed_spends) / (one_copy_cost - var_spends)
print("Xd =", math.ceil(Xd))
print(
    "Выводы. Объем продаж для получения дополнительной прибыли в размере", additional_profit,
